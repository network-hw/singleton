...
%token <Support.Error.info> MVAR
%token <Support.Error.info> MMVAR
...
%token <Support.Error.info> EQLLT
%token <Support.Error.info> KNDU
%token <Support.Error.info> KNDA
...
/* An "arrow type" is a sequence of atomic types separated by
   arrows. */
ArrowType :
    AType ARROW LPAREN Kind RPAREN ArrowType 
...
Term :
  ...
  | AppTerm EQLLT AppTerm
      { fun ctx -> TmAssignMvar($2, $1 ctx, $3 ctx) }
  | FORK TermProduct
...
AppTerm :
  ...
  | MVAR PathTerm
      { fun ctx -> TmMvar($1, $2 ctx) }
  | BANG PathTerm 
      { fun ctx -> TmDeref($1, $2 ctx) }
  | TRIANGLE PathTerm
      { fun ctx -> TmDeMvar($1, $2 ctx) }
...
TermProduct:
    LCURLY Term COMMA Term RCURLY
      { fun ctx -> TmProduct($1, $2 ctx, $4 ctx) }
