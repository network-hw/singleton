...
let tymap onvar c tyT = 
  ...
  | TyId(b) as tyT -> tyT
  | TyArr(tyT1,tyT2, knd) -> TyArr(walk c tyT1,walk c tyT2, knd)
  | TyTop -> TyTop
  | TyBool -> TyBool
  ...
  | TyNat -> TyNat
  | TyMvar(tyT1) -> TyMvar(walk c tyT1)
  | TyProduct(tyT1,tyT2) -> TyProduct(walk c tyT1, walk c tyT2)
  in walk c tyT
...
let tmmap onvar ontype c t = 
  ...
  | TmMvar(fi, t1) -> TmMvar(fi, walk c t1)
  | TmDeMvar(fi,t1) -> TmMvar(fi, walk c t1)
  | TmAssignMvar(fi,t1,t2) -> TmAssignMvar(fi, walk c t1, walk c t2)
  | TmProduct(fi,t1,t2) -> TmProduct(fi, walk c t1, walk c t2)
  | TmFork(f1,t) -> TmFork(f1, walk c t)
  in walk c t
    ...
    match getbinding fi ctx i with
      ...
      | _ -> error fi 
        ("getTypeFromContext: Wrong kind of binding for variable " 
          ^ (index2name fi ctx i)) 
  with GetTypeFailure -> raise GetTypeFailure
(* ---------------------------------------------------------------------- *)
...
let printknd knd = match knd with
    KndA -> pr "A"
  | KndU -> pr "U"
...
let rec printty_Type outer ctx tyT = match tyT with
  ...
  | TyMvar(tyT) -> pr "Mvar "; printty_AType false ctx tyT
...
let prcontext ctx = 
  pr "---- length: "; pr (string_of_int (List.length ctx)); pr "\n";
  let rec walk ctx = match ctx with
    ...
    | (s, TmAbbBind(t,tyT))::ctx' -> 
      ...
      (match tyT with 
          None -> pr "None\n"
        | Some(ty) -> printty ctx ty; pr "\n");
      walk ctx';
    | (s, _)::ctx' -> pr "other: "; pr s; pr "\n"; walk ctx';
  in
  walk ctx;
  pr "----\n";
